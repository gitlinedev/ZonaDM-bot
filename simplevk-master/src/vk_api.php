...
class vk_api {
     ...
     * @throws VkApiException
     */
    public function isAdmins($user_id, $chat_id) { //возвращает привелегию по id
        try {
    ...
    public function isAdmin($user_id, $chat_id) { //возвращает привелегию по id
        ...
        foreach ($members as $key) {
            if ($key['member_id'] == $user_id)
                return ((isset($key["is_admin"])) ? 'admin' : false);
        }
        return null;
    }
    public function isOwner($user_id, $chat_id) { //возвращает привелегию по id
        try {
            $members = $this->request('messages.getConversationMembers', ['peer_id' => $chat_id])['items'];
        } catch (\Exception $e) {
            throw new VkApiException('Бот не админ в этой беседе, или бота нет в этой беседе');
        ...
        foreach ($members as $key) {
            if ($key['member_id'] == $user_id)
                return ((isset($key["is_owner"])) ? 'owner' : false);
        }
        return null;
    }
    /**
    ...
    public function eventAnswerSnackbar($text, $user_id) {
    ...
    public function generateCarousel($carousels, $id) {
        ...
        foreach ($carousels as $carousel) {
            ...
            if (isset($carousel['kbd']))
                $element['buttons'] = $this->generateKeyboard([$carousel['kbd']])[0];
            ...
            foreach ($button_str as $button) {
                ...
                switch ($button[0]) {
                    ...
                    case 'vkpay': {
                        ...
                        $keyboard[$i][$j]["action"]["hash"] .= (isset($button[5])) ? "&description={$button[5]}" : '&description=Оплата';
    ...
    public function sendDocMessage($id, $local_file_path, $title = null, $params = []) {
        ...
        $upload_file = $this->uploadDocsMessages($id, $local_file_path, $title);
        if ($id != 0 and $id != '0') {
            return $this->request('messages.send', ['attachment' => "doc" . $upload_file['doc']['owner_id']. "_" . $upload_file['doc']['id'], 'peer_id' => $id] + $params);
        } else {
            return true;
        }
    }
